...
def issue_verification_token_and_send(db: Session, user: User) -> tuple[str, datetime]:
    raw_token, expires_at = generate_email_verification_token()
    user.email_verification_token = hash_token(raw_token)
    user.email_verification_expires_at = expires_at.replace(tzinfo=None)
    db.commit()
    db.refresh(user)

    sent = send_verification_email(user.email, raw_token)
    print(
        f"[EMAIL_VERIFY] {user.email} token={raw_token} expires_at={expires_at.isoformat()} sent={sent}"
    )
...
def register(payload: RegisterRequest, db: Session = Depends(get_db)):
    ...
    issue_verification_token_and_send(db=db, user=user)
...
@router.post("/verify-login", response_model=VerifyLoginResponse)
def verify_email_and_login(payload: VerifyEmailRequest, db: Session = Depends(get_db)):
    token = payload.token.strip()
    if not token:
        raise HTTPException(status_code=400, detail="Missing token")
    ...
    return VerifyLoginResponse(ok=True, access_token=access_token)
